...
void CompilationEngine::compileClass(){
	...
	while (jt->tokenType() == tKEYWORD && (jt->keyword() == kFIELD || jt->keyword() == kSTATIC))
		compileClassVarDec();
	
	 while (jt->tokenType() == tKEYWORD && (jt->keyword() == kCONSTRUCTOR
           || jt->keyword() == kFUNCTION || jt->keyword() == kMETHOD))
		compileSubroutine();
...
void CompilationEngine::writeIdentifier() {
    ...
    if (kind != ST::NONE) {
        type = st->typeOf(name);
        //cout  << "<identifier type=" << type << "kind="  << st->fromKind(kind) <<  "> " << name << " </identifier>";

        write( "<identifier type=" + type + " kind="  + st->fromKind(kind) +  " index=" + to_string (st->indexOf(name)) + "> " + name + " </identifier>" );
    }
...
void CompilationEngine::compileSubroutine(){
	...
	if (jt->keyword() == kCONSTRUCTOR) {
        ...
        st->define("this",className_,ST::ARG); // first argument of a method is always "this"
		eat("method");
    }
...
void CompilationEngine::compileStatements(){
	...
	//cout << "kkk "<< jt->currentToken <<  isStatement(jt->currentToken) <<  (jt->tokenType() == tKEYWORD) << endl;
...
void CompilationEngine::compileReturn(){
    ...
    if (jt->tokenType() != tSYMBOL || (jt->tokenType() == tSYMBOL && jt->symbol() != ';')){
        compileExpression();
        vm->writeReturn();
    }
    else {
...
void CompilationEngine::compileTerm() 
{
    ...
    switch(jt->tokenType()) {
        ...
        case tIDENTIFIER:
            ...
            } else if (jt->tokenType() == tSYMBOL && (jt->symbol() == '.' || jt->symbol() == '(')) {
...
void CompilationEngine::writeTerminal(string terminal)
{
    write(terminal);
}
...
void CompilationEngine::write(string s) {
	out << s << endl;
}
